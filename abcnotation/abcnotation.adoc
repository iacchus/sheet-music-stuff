...
link:[]An *abc file* consists of one or more abc tune transcriptions,
...
link:[]An abc file with more than one tune in it is called an **abc
tunebook**.
...
link:[]An *abc tune* itself consists of a tune header and a tune body,
terminated by an empty line or the end of the file. It may also contain
comment lines or stylesheet directives.

link:[]The *tune header* is composed of several information field lines,
...
link:[]The **tune body**, which contains the music code, follows
...
link:[]Abc *music code* lines are those lines in the tune body which
...
link:[]**Free text** is just that. It can be included anywhere in an abc
...
link:[]**Typeset text** is any text specified using text directives. It
...
link:[]Traditionally abc has been used in dedicated abc files. More
...
An link:[]**abc fragment** is a partial abc tune. It may contain a
...
* link:[]a **code line-break**, meaning a line-break in the abc tune
...
link:[]Text written within an abc file, either as part of an information
